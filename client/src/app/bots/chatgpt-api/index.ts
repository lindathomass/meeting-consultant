...
export abstract class AbstractChatGPTApiBot extends AbstractBot {
  ...
  buildMessages(prompt: string): ChatMessage[] {
    ...
    const systemMessage = this.getSystemMessage().replace(
      "{current_date}",
      currentDate
    );
    return [
...
export class ChatGPTApiBot extends AbstractChatGPTApiBot {
  constructor(
    private config: Pick<
      ...
      | "openaiApiHost"
      | "chatgptApiModel"
      | "chatgptApiTemperature"
      | "chatgptApiSystemMessage"
    >
  ) {
    super();
  }
  ...
  getSystemMessage() {
    return (
      this.config.chatgptApiSystemMessage || DEFAULT_CHATGPT_SYSTEM_MESSAGE
    );
  }
  ...
  async fetchCompletionApi(messages: ChatMessage[], signal?: AbortSignal) {
    ...
    if (!resp.ok && resp.status === 404 && chatgptApiModel.includes("gpt-4")) {
      throw new ChatError(
        `You don't have API access to ${chatgptApiModel} model`,
        ErrorCode.GPT4_MODEL_WAITLIST
      );
    }
    if (!resp.ok) {
      ...
      if (error.includes("insufficient_quota")) {
        throw new ChatError(
          "Insufficient ChatGPT API usage quota",
          ErrorCode.CHATGPT_INSUFFICIENT_QUOTA
        );
      }
    }
    return resp;
