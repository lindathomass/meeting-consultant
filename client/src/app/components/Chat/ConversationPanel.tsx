...
const ConversationPanel: FC<Props> = (props) => {
  ...
  if (props.generating) {
    ...
    inputActionButton = (
      <div className="flex flex-row gap-[10px] shrink-0">
        <Button text="Send" color="primary" type="submit" />
      </div>
    );
  }
  ...
  return (
    <ConversationContext.Provider value={context}>
      <div
        className={cx(
          "flex flex-col overflow-hidden bg-primary-background h-full rounded-2xl"
        )}
      >
        <div
          className={cx(
        ...
        >
          <div className="flex flex-row items-center">
            ...
            <Tooltip content={props.bot.name || botInfo.name}>
              <span className="font-semibold text-primary-text text-sm cursor-default ml-2 mr-1">
                {botInfo.name}
              </span>
            </Tooltip>
          ...
          <div className="flex flex-row items-center gap-3">
            ...
            <Tooltip content={"Clear conversation"}>
              <motion.img
                ...
                className={cx(
                  "w-5 h-5",
                  props.generating ? "cursor-not-allowed" : "cursor-pointer"
                )}
        ...
        >
          ...
          >
            {mode === "compact" && (
              <span className="font-medium text-xs text-light-text cursor-default">
                Send to {botInfo.name}
              </span>
            )}
