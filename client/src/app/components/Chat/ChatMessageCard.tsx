...
const COPY_ICON_CLASS =
	"self-top cursor-pointer invisible group-hover:visible mt-[12px] text-primary-text";
		...
		>
			<div className="flex flex-col w-11/12  max-w-fit items-start gap-2">
				<MessageBubble color={message.author === "user" ? "primary" : "flat"}>
					...
					{!!message.error && (
						<p className="text-red-500">{message.error.message}</p>
					)}
				</MessageBubble>
			...
			{!!copyText && (
				<CopyToClipboard text={copyText} onCopy={() => setCopied(true)}>
					{copied ? (
						<IoCheckmarkSharp className={COPY_ICON_CLASS} />
					) : (
						<IoCopyOutline className={COPY_ICON_CLASS} />
					)}
				</CopyToClipboard>
			)}
		</div>
