  ...
  return (
    <div className="rounded-xl bg-secondary h-9 flex flex-row items-center px-4 grow">
      ...
      <input
        ...
        placeholder={`${"Search"} ${
          props.disabled ? `(${"Premium Feature"})` : ""
        }`}
        value={props.value}
...
const HistoryDialog: FC<Props> = (props) => {
  ...
  const clearAll = useCallback(async () => {
    if (confirm("Are you sure you want to clear history messages?"!)) {
  ...
  return (
    ...
    >
      <div className="border-b border-solid border-primary-border pb-[10px] mx-5 flex flex-row items-center gap-4">
        <Tooltip content={"Clear history messages"}>
          <div
            className="bg-secondary p-2 rounded-xl cursor-pointer"
            onClick={clearAll}
          >
            <VscClearAll size={18} className="opacity-80" />
          </div>
        </Tooltip>
        <SearchInput disabled={true} value={keyword} onChange={setKeyword} />
      </div>
